...
namespace NortwindAdmin
{
    public partial class FormClientes : Form
    {
        ...
        public FormClientes(CustomerService customerService, OrderService orderService)
        {
            InitializeComponent();
        ...
        private async void btnRegistrar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("El ID del cliente debe tener exactamente 5 caracteres.",
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show(mensaje, "Error al registrar",
        ...
        private async void btnBuscar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var lista = await _customerService.SearchCustomers(texto);
        ...
        private async void dgvClientes_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex >= 0)
                {
                    string customerId = dgvClientes.Rows[e.RowIndex].Cells["CustomerID"].Value?.ToString() ?? "";

                    txtIdCliente.Text = customerId;
                    txtEmpresa.Text = dgvClientes.Rows[e.RowIndex].Cells["CompanyName"].Value?.ToString() ?? "";
                    txtContacto.Text = dgvClientes.Rows[e.RowIndex].Cells["ContactName"].Value?.ToString() ?? "";
                    txtCiudad.Text = dgvClientes.Rows[e.RowIndex].Cells["City"].Value?.ToString() ?? "";

                    var pedidos = await _orderService.GetOrdersByCustomerId(customerId);
        ...
        private void groupBoxBotones_Enter(object sender, EventArgs e)
        {
